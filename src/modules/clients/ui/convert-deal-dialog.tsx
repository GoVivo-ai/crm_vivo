...
export function ConvertDealDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button />}>Marcar ganado</DialogTrigger>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
          <p className="text-sm text-muted-foreground">
            ...
            cliente. Marca los servicios que se contratan desde hoy (puedes
            agregarlos después en la vista 360).
          </p>
          {catalog.length > 0 && (
          ...
          <Button type="submit" disabled={pending}>
            {pending ? "Convirtiendo…" : "Confirmar cierre ganado"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
